...
app = FastAPI(title="Hedge AI API", version="1.0")
...
@app.post("/chat")
def api_chat(prompt: Prompt):
    return {"response": chat(prompt.text)}

@app.get("/docs-json")
def custom_openapi():
    return get_openapi(title="Hedge AI API", version="1.0", routes=app.routes)

threading.Thread(target=train_every_hour, daemon=True).start()
threading.Thread(target=lambda: uvicorn.run(app, host="0.0.0.0", port=8000), daemon=True).start()

gr.Interface(fn=chat, inputs="text", outputs="text", title="Hedge AI", description="Self-trained DeepSeek AI with auto-learning and API").launch()
